...
# 读取Excel文件（你训练生成的文件，完全不变）
df = pd.read_excel('tcn_model_metrics.xlsx')

# 提取需要绘制的数据（只取原始数据，不取平滑）
...
# ===================== 只画原始曲线，无平滑、无多余处理 =====================
# 子图1：验证集准确率
ax1.plot(epochs, val_accuracy, color='#1f77b4')
...
# 子图2：精确率
ax2.plot(epochs, precision, color='#ff7f0e')
...
# 子图3：召回率
ax3.plot(epochs, recall, color='#2ca02c')
...
# 子图4：F1分数
ax4.plot(epochs, f1_score, color='#d62728')
...
# 设置整个画布的总标题
fig.suptitle('TCN模型训练关键指标变化曲线（原始数据）', fontsize=20, y=0.98)

# 调整子图间距
plt.tight_layout(rect=[0, 0, 1, 0.96])

# 保存高分辨率图片
plt.savefig('tcn_training_raw_curves.png', dpi=300, bbox_inches='tight')
