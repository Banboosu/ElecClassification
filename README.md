...
标签定义、真实内部数据来源及既有 TCN 部署依据见 `docs/data_dictionary.md`。
...
正式训练统一在 Linux x86_64 CUDA 服务器执行。本地只建议进行代码和数据检查，不运行完整模型
训练。依赖配置将普通 Python 包固定到阿里云 PyPI 镜像，并且仅将 PyTorch 固定到阿里云 CUDA
12.6 镜像；CUDA 12.6 构建包含 Tesla V100 所需的 `sm_70` 支持。
...
首次同步前请确保已经拉取最新的 `pyproject.toml` 和 `uv.lock`。不要通过普通 PyPI 单独重装
`torch`，否则 Linux 环境可能被替换为不支持 V100 的 CUDA 13 构建。
...
### 4. V100 报告 `compute capability 7.0` 不受支持

这表示安装成了 CUDA 13 版 PyTorch。同步最新依赖配置后重建虚拟环境：
