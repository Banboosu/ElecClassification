...
1. **预训练归因：**比较预训练与随机初始化 MOMENT 的冻结表征，明确低标签优势是否来自预训练。
2. **生产兼容离线回放：**保持既有 TCN 输入、标签与输出约束，对照旧 TCN 和候选 MOMENT，
   量化效果、模型分歧与端到端成本，不修改线上模型。
3. **早期检测与鲁棒性：**只输入前 25%/50%/75% 曲线，并测试噪声、缺失和采样变化下的 FNR/FPR。
4. **漏检与误报复盘：**人工检查完整标签 TCN、Random Forest 和专用 MOMENT 的共同错误，
   归纳早期信号不足、类别曲线重叠、阈值不匹配和非典型曲线等失效模式。
5. **CNN 一致性：**现有 CNN 五种子来自 AMP 配置，建议用最终 FP32 代码低成本补跑。
6. **全局标准化：**增加只用训练集均值/方差的全局标准化，量化幅值信息和数值尺度的影响。
