...
3. **MOMENT 需要与任务匹配的适配方式。**冻结线性头、最后两层微调、冻结表征 RBF-SVM 和
   完全微调的 Macro-F1 依次为 77.41%、81.50%、85.17% 和 95.37%。非线性下游分类器
   优于浅层解冻，而充分微调才能释放接近 TCN 的任务上限。
4. **跨任务收益具有明确边界。**MOMENT 零样本插补在所有 8 个缺失条件下均未超过线性插值；
...
使用 MOMENT 时采用完全微调；跨任务零样本应用必须单独验证。

## 1. 研究问题与实验结构

| 编号 | 研究问题 | 对应实验 |
|---|---|---|
| RQ0 | 数据是否存在无效标签、重复泄漏和长度捷径？ | 数据质量审计、长度决策树 |
...
标签 5 的最终业务含义仍需由数据提供者确认。论文应表述为“依据数据协议过滤”，不能仅根据
模型效果反向定义其含义。
...
数据没有可重复设备 ID 和时间戳，因此当前只能验证随机分层划分下的泛化，不能宣称跨设备或
跨时间泛化。

### 2.3 长度捷径审计

仅使用序列长度训练深度为 3 的决策树，测试结果为 45.60% Accuracy 和 35.76% Macro-F1，
高于多数类的 36.94% Accuracy，说明长度与类别有关。所有深度模型均使用 mask 排除 padding；
后续无监督检索还单独加入 Length Only 基线。长度信息可能包含真实充电阶段差异，也可能形成
采集或标注捷径，结论中必须保留这一不确定性。
...
逐序列 z-score 和 minmax 会消除样本间绝对功率幅值，而 raw TCN 最好，说明绝对幅值很可能是
有效判别信息。它也可能反映设备量程或采集条件，因此结果支持“保留幅值”，但不能证明幅值一定
来自真实故障机制。训练集全局标准化仍是值得补充的对照。
...
| 研究问题 | 实验回答 |
|---|---|
| RQ0 数据是否可靠？ | 过滤后无重复泄漏和冲突标签，但长度与类别相关，且缺少设备/时间字段。 |
...
| RQ4 预训练是否节省标签？ | 是。1%–10% 标签时 MOMENT 更准、更稳定；40% 标签时 TCN 反超。 |
...
6. 无监督检索，作为分类之外的表征复用证据；
7. 零样本插补负结果与迁移边界；
8. 工程优化、数值稳定性、研究限制与未来工作。

## 14. 局限性与后续实验优先级

1. **划分限制：**应优先获取设备 ID 和时间戳，再补充 group split 或 time split。
2. **标签依据：**需要数据提供者解释标签 5 的产生机制和类别 0/1/2 的业务定义。
3. **CNN 一致性：**现有 CNN 五种子来自 AMP 配置，建议用最终 FP32 代码低成本补跑。
4. **全局标准化：**增加只用训练集均值/方差的全局标准化，区分幅值信息和数值尺度影响。
