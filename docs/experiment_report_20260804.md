...
6. **模型错误受长度影响且具有不同结构。**五种模型在最长序列区间均出现相对下降；TCN 的
   主要类别混淆为 0→1、1→0、2→0，完全微调 MOMENT 为 0→2、1→2、2→0。固定规则曲线
   显示，共同错误、仅 TCN 错误和仅 MOMENT 错误对应不同的长度、斜率或转折率特征。

综合而言，本文不支持“用大模型替代所有专用模型”的结论，而支持一种更具体的选择原则：
低标签冷启动优先使用冻结 MOMENT 表征，标签充足且关注已测训练侧资源时使用 TCN，算力充足且必须
...
| RQ7 | 取得效果需要多少算力？ | 训练/拟合时间、峰值显存和参数量对照 |
| RQ8 | 电池异常能否同时少漏检、少误报？ | one-vs-rest、PR-AUC、验证集高召回阈值 |
| RQ9 | 模型主要错在哪里，是否受序列长度影响？ | 逐样本类别分数、混淆矩阵、长度分层、固定规则错误曲线 |
...
| 1D-CNN（逐序列 z-score） | 5/5 seeds | 正式主表；M03 AMP 稳定性审计通过 |
...
| M02 轻量错误分析 | 25 个模型—seed 预测文件 | 正式混淆、长度分层和错误曲线 |
| M03 最终协议审计 | 105/105 正式运行 | 划分、指标、CNN AMP 与主张边界均通过 |
...
| 1D-CNN（逐序列 z-score） | 94.29 ± 1.27 | 94.45 ± 1.21 | 94.38 ± 1.27 | AMP；M03 审计通过 |
...
TCN（原始尺度）相对随机森林提高 7.09 个百分点，配对 95% t 区间为 [6.21, 7.98]；五个 seed
...
复用五个 seed 的已有 checkpoint，导出完整标签 TCN、完全微调 MOMENT 以及 1%/5%/10%
MOMENT-SVM 的 validation/test 逐样本预测和类别分数。共得到 25 个预测文件、263,250 行记录，
其中测试集 175,500 行；完整逐 seed 与聚合混淆矩阵均已保存。
...
区间由 seed 42 全部有效输入的长度四分位点固定，不读取标签。所有方案在最长区间均相对较弱。
TCN 的每类主要错误方向为 0→1、1→0、2→0，完全微调 MOMENT 为 0→2、1→2、2→0。
运行内预测标签 2 的置信边际前 10% 中，TCN、完全微调 MOMENT、1%/5%/10% MOMENT-SVM
平均分别有 0.4、0.8、16.0、7.8、6.8 个高置信误报。

固定 seed 42 和六条预声明规则得到电池漏检、高置信误报、共同错误、仅 TCN 错误、仅 MOMENT
错误和仅 10% SVM 错误各一条曲线。五种子测试单元中，共同错误 585 个、仅 TCN 错误 839 个、
仅 MOMENT 错误 1,079 个；相对两模型均正确组，三组分别突出更高转折率/极差、更长有效长度和
更高转折率等描述性特征。
...
|---|---:|---:|---:|
| 1D-CNN（逐序列 z-score） | 136.3 ± 51.7 s/次 | 63 MiB | 51,971 参数 |
| TCN（原始尺度） | 345.5 ± 79.4 s/次 | 267 MiB | 218,691 参数 |
...
以上计算成本只描述已观测的训练或拟合时间、峰值 GPU 显存以及参数量或训练规模。当前没有统一
测量线上推理延迟、吞吐、能耗或端到端生产速度，因此这些训练侧结果不能外推为部署推理速度。
...
| RQ1 完整标签谁最好？ | TCN 均值最高且已测训练侧资源权衡最佳；完全微调 MOMENT 效果接近但成本高。 |
...
| RQ9 错误是否受长度与曲线形态影响？ | 是。最长序列是所有方案的相对薄弱区间；TCN 与完全微调 MOMENT 的主要混淆方向和错误曲线特征不同。 |

## 14. 论文主张、边界与推荐结构

### 14.1 可以支持的主张

1. TCN 在本数据的完整标签分类中达到最高均值，并具有更好的已测训练侧资源权衡。
...
8. 五随机种子的逐样本复评显示，最长序列应作为模型复核的重点，TCN 与 MOMENT 的错误不能
   视为完全同构。
...
3. **漏检与误报人工复盘：**M02 已按固定规则完成自动分析；若具备业务人员时间，再人工检查
   完整标签 TCN、Random Forest 和专用 MOMENT 的代表性共同错误与模型分歧。
4. **全局标准化：**增加只用训练集均值/方差的全局标准化，量化幅值信息和数值尺度的影响。
5. **参数高效微调：**若论文篇幅和算力允许，可测试解冻最后 8/12 层或 LoRA；先用 seed42
   门槛试验，验证 Macro-F1 超过 90% 后再扩展五种子。
6. **检索人工评价：**构建少量人工相似度标注或业务相关性问卷，验证类别纯度代理是否合理。
7. **目标域插补：**可微调 reconstruction head 或执行自监督遮挡适配，但必须与 zero-shot
   结果分开报告。
8. **统计限制：**五随机种子区间不适合过强推断，多个消融比较应考虑多重比较和更多划分。
...
- [M02 固定规则典型错误曲线](figures/m02_typical_error_curves_20260805.png)
...
- M02 轻量错误分析结果包：`artifacts/imports/m02_error_analysis_20260805.tar.gz`
- M03 最终协议审计包：`artifacts/imports/m03_protocol_audit_20260805.tar.gz`
