...
充电功率曲线能够在不增加额外传感器的条件下反映充电过程状态，但真实故障标注成本高，且
“正常—充电器故障—电池异常”分类同时受到变长输入、类别相关的序列长度和安全关键误报—漏检
权衡影响。为检验通用时序预训练能否在该场景中优于专用模型，本文在 35,099 条有效充电功率
序列上，对多数类、统计特征逻辑回归、随机森林、1D-CNN、时序卷积网络（TCN）和
...
Charging-power curves provide a non-intrusive signal for characterizing charging states, but fault
labels are expensive and the classification of normal, charger-fault, and battery-abnormal sessions
...
sequences. Majority prediction, statistical-feature logistic regression and random forest, a 1D-CNN,
...
原始数据包含唯一记录 ID、类别标签和以逗号分隔的单变量充电功率序列。项目当前采用的标签约定
为：0 表示正常、1 表示充电器故障、2 表示电池异常、5 表示无效或不完整记录。该语义来自既有
项目代码和配置，尚需数据提供者给出正式标注规则，故本文称其为“项目标签约定”，不将其描述为
已由行业标准或独立专家验证的诊断结论。
...
标签 5 中有 10,613 条序列短于 18 个点，中位长度为 1，且 9,101 条为恒定序列。这些统计支持
将其与有效三分类数据分离，但不能替代业务来源核验。过滤后的有效数据未发现完全重复序列或冲突
标签，仅有 4 条恒定序列。
...
当前 `charging_station_id` 在文件中逐行唯一，不能作为设备分组字段；数据也不包含可用时间戳。
因此当前结果只证明随机分层划分下的内部泛化，不能证明跨设备、跨站点或跨时间泛化。
...
提示幅值可能是有效判别信号；但幅值也可能编码设备量程或采集条件。投稿前需要用训练集全局
标准化和分组外推实验区分这两种解释（实验 E06、E09）。
...
因此，本文将安全专项结果视为模型选择约束，而不是 MOMENT 优势。任何上线主张还需要业务确认的
标签、漏检/误报成本、前缀早期检测、概率校准、跨设备验证和人工错误复核。目前结果只适合作为
离线研究证据。
...
1. 标签语义、采样间隔、标注来源和标签 5 的产生机制尚未获得数据提供者正式确认。
2. 缺少设备 ID、站点 ID 和时间戳，只完成了随机分层划分，外部有效性不足。
3. 当前只有单一私有数据集和五个随机种子；配对区间未进行多重比较校正。
4. 少标签比较使用各自既定最佳协议，但预处理、分类器和调参预算并不完全相同。
5. 缺少 ROCKET/MiniROCKET、TS2Vec 和其他时序基础模型等强通用表征基线。
6. 尚未完成通用三分类错误样例、噪声/重采样/截断鲁棒性、概率校准和按序列长度分层分析。
7. CNN 五随机种子来自 AMP 配置，尚需用最终 FP32 协议低成本复跑以统一数值设置。
8. 尚未测量各模型在统一 batch 和硬件下的推理延迟、吞吐与能耗。
9. 检索以类别一致性代理业务相关性，尚无人工作出的相似度标注。
...
跨设备/时间外推和错误复核后，方可将当前单数据集结论提升为更一般的工程结论。

## 数据与代码可用性

原始数据包含【待补：数据权属/隐私说明】，当前不能随稿公开。实验代码、配置和不含原始序列的
派生指标能否公开由【待补：数据提供者/项目负责人】确认。投稿版本应提供匿名仓库或受控访问说明，
并列出数据哈希和正式实验产物清单。

## 伦理声明

本研究不涉及【待确认：人体受试者/个人身份信息】。数据脱敏方式和允许披露范围尚需数据提供者
确认。本文模型不构成安全认证或自动控制依据。
